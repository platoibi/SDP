...
public class GattClient : MonoBehaviour
{
    ...
    public static Quaternion cubeRot=new Quaternion(0, 0, 0, 1);
    public static Vector3 cubePos = new Vector3(0, 0, 0);
    public static float right;
    public static float left;
    public static float up;
    public static float straight;
    ...
    int cnt = 0;
    private void DumpValue(Byte[] Value, UInt16 Handle)
    {
        ...
        else
        {
            ...
            if (Handle.ToString("X4") == "000B")
            {
                left = BitConverter.ToSingle(Value, 0);
                arr[0] = left;
                cnt++;
            }
            else if (Handle.ToString("X4") == "000E")
            {
                right = BitConverter.ToSingle(Value, 0);
                arr[1] = right;
                cnt++;
            }
            else if (Handle.ToString("X4") == "0011")
            {
                straight = BitConverter.ToSingle(Value, 0);
                arr[2] = straight;
                cnt++;
            }
            else if (Handle.ToString("X4") == "0014")
            {
                up = BitConverter.ToSingle(Value, 0);
                arr[3] = up;
                cnt++;
            }
            else
            {
                acceleration = BitConverter.ToSingle(Value, 0);
                arr[4] = acceleration;
                cnt++;
            }
            if ( cnt == 5)
            {
                cnt = 0;
                Magnus.magnus.Shoot(arr[2], arr[3], arr[0], arr[1], arr[4]);
            }
            //Debug.Log(Handle.ToString("X4") + " : " + BitConverter.ToSingle(Value, 0));
        }
    }
    ...
    private void FManager_OnDiscoveringCompleted(object Sender, wclBluetoothRadio Radio, Int32 Error)
    {
        ...
        else
        {
            ...
            Debug.Log("Try to resolve devices name. Looking for device with " + deviceName + " as its name");
            Int64 Device = 0;
            ...
            else
            {
                ...
                Debug.Log("Device address is :" + Device);
